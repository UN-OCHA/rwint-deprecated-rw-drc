...
wax.tilejson(baseUrl + layers.join(',') + (',') + datalayers.join(',') + '.jsonp',
function(tilejson) {
    tilejson.minzoom = 2;
    tilejson.maxzoom = 7;
...
$(function() {
    ...
    var famineSlider = new Dragdealer('slider', {
        ...
        animationCallback: function(x, y) {
            ...
            dataCtrl[pos] && $('#slide-bar').html(dataCtrl[pos].month);
    ...
    function() {
        var selection = layerselect.selection();
        selection.each(function(data) {
            datalayers.push(data[slider.month.data]);
        });.
        ...
        $('ul.li a').click(function(e) {
            e.preventDefault();
            // Remove the active class on comparisons if there is one
            $('ul.comparisons li a').removeClass('active');

            var thisParent = $(this).closest('li');
            if (!$(this).hasClass('active')) {
                // Grab our data attributes
                var activeLayers = $(this).attr('data-layer').split(',');
                var start = $(this).attr('data-start');
                var end = $(this).attr('data-end');

                $('#timeline span.start').text(start);
                $('#timeline span.end').text(end);

                if (!thisParent.hasClass('active')) {
                    $('ul.navigation li ul li').removeClass('active');
                    thisParent.addClass('active');
                    // Remove the old theme class first
                    $('#timeline').removeClass(theme);
                    theme = $(this).attr('data-theme');
                    mapEventsOnClick(activeLayers);
                }
            }
        });
